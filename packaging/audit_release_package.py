...
FORBIDDEN: list[tuple[str, re.Pattern[bytes], str]] = [
    ...
    ("vendor License Manager", re.compile(rb"zenith_license_manager"),
     "the vendor License Manager issues licences and must not ship"),
    ("vendor signing key file", re.compile(rb"zenith-vendor-signing-key|ZKEY1"),
     "an encrypted vendor signing key must never leave the vendor's machine"),
    ("vendor licence history", re.compile(rb"license-history\.json"),
     "the issue history holds customer contact details and must not ship"),
